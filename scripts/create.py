...
# 設置日誌
def setup_logger(name: str = "project-memory-manager", workspace: Path = None) -> logging.Logger:
    """設置日誌記錄器"""
    if workspace is None:
        # 嘗試從環境變量讀取工作空間
        workspace_env = os.environ.get("PROJECT_MEMORY_WORKSPACE")
        if workspace_env:
    ...
    log_dir = workspace / "projects" / "_logs"
...
class ProjectManager:
    ...
    def __init__(self, workspace: Optional[Path] = None):
        if workspace is None:
            ...
            workspace_env = os.environ.get("PROJECT_MEMORY_WORKSPACE")
            if workspace_env:
                self.workspace = Path(workspace_env)
            else:
                # 基於技能目錄位置推測
                skill_dir = Path(__file__).parent.parent
                # 檢查是否是技能目錄結構（包含projects/和scripts/）
                if (skill_dir / "projects").exists() and (skill_dir / "scripts").exists():
                    self.workspace = skill_dir
                else:
                    # 否則假設技能安裝在workspace的skills/目錄下
                    self.workspace = skill_dir.parent
        else:
        ...
        # 初始化日誌
        self.logger = setup_logger(workspace=self.workspace)
    ...
    def manual_create(self, project_name: str, description: str = "") -> Path:
        """手動創建項目"""
        self.logger.info(f"手動創建項目: {project_name}")
        ...
        if project_path.exists():
            self.logger.warning(f"項目已存在: {project_path}")
            return project_path
        ...
        self.logger.info(f"✅ 項目創建成功: {project_name}")
        self.logger.info(f"📁 位置: {project_path}")
    ...
    def create_project_files(self, project_path: Path, project_name: str, description: str):
        ...
        for filename in file_list:
            ...
            self.logger.debug(f"創建文件: {output_file}")
        
        # 創建Attachment資料夾（用於存放其他文件）
        attachment_dir = project_path / "Attachment"
        attachment_dir.mkdir(exist_ok=True)
        self.logger.info(f"📁 創建Attachment資料夾: {attachment_dir}")
...
"""
        ...
        self.logger.info(f"創建默認模板: {templates_dir}")
    
    def update_index(self, project_config: Dict[str, Any]):
        """更新INDEX.md"""
        if not self.index_file.exists():
            self.logger.error(f"索引文件不存在: {self.index_file}")
            return
        ...
        try:
            ...
            if existing_line != -1:
                ...
                self.logger.info(f"更新索引行: {project_config['name']}")
            else:
                # 插入新行（在表格標題後）
                ...
                self.logger.info(f"添加索引行: {project_config['name']}")
            
            # 檢查token數量
            new_content = "\n".join(lines)
            if self.estimate_tokens(new_content) > self.config["index"]["max_tokens"]:
                self.logger.warning(f"索引文件超過{self.config['index']['max_tokens']}tokens，建議清理")
            
            # 保存
            self.index_file.write_text(new_content, encoding='utf-8')
            self.logger.info(f"索引更新完成: {self.index_file}")
            
        except Exception as e:
            self.logger.error(f"更新索引失敗: {e}")
    
    def analyze_memory(self, threshold_tokens: int = 500) -> List[Dict[str, Any]]:
        """分析MEMORY.md，識別超過閾值的主題"""
        if not self.memory_file.exists():
            self.logger.warning(f"MEMORY.md不存在: {self.memory_file}")
            return []
        ...
        try:
            ...
            self.logger.info(f"發現 {len(topics)} 個超過{threshold_tokens}tokens的主題")
            return topics
            
        except Exception as e:
            self.logger.error(f"分析MEMORY.md失敗: {e}")
            return []
    
    def auto_migrate(self, threshold_tokens: int = 500, dry_run: bool = False) -> List[Path]:
        """自動遷移超過閾值的主題到項目"""
        self.logger.info(f"自動遷移開始 (閾值: {threshold_tokens}t, 乾跑模式: {dry_run})")
        ...
        for i, topic in enumerate(topics):
            ...
            self.logger.info(f"遷移主題 {i+1}: {project_name} ({topic['tokens']}t)")
            ...
            if not dry_run:
                try:
                    ...
                    self.logger.info(f"✅ 遷移完成: {project_name}")
                    
                except Exception as e:
                    self.logger.error(f"遷移失敗: {e}")
            else:
                self.logger.info(f"乾跑: 將創建項目 '{project_name}'")
        ...
        if not dry_run and migrated_projects:
            ...
            self.logger.info(f"🎉 自動遷移完成，創建了 {len(migrated_projects)} 個項目")
            ...
            if project_dir.is_dir() and not project_dir.name.startswith('_'):
                ...
                if config_file.exists():
                    try:
                        ...
                        self.logger.error(f"讀取項目配置失敗 {project_dir}: {e}")
...
"""
        ...
        self.logger.info(f"✅ 索引重建完成，包含 {len(projects)} 個項目")
...
def main():
    ...
    # 添加全局工作空間參數
    parser.add_argument("--workspace", help="工作空間目錄（絕對路徑），如未指定則自動檢測")
